    ...
    )
    {
        ...
        if(isSelfPlay==false) {
            if(generateNetwork==true) {
                ...
                friendlyName = "TrainAgent"+Configuration.mcts_it;
            ...
            if(noReset==false) {
                ...
                network.closeModel();
                Move bestAction = mcts.getBestAction(node);
...
//                    System.out.println(node.getParent());
    ...
    public TrainIter getActionProbability(Game game, int temp){
        ...
        float [][][] stateTensor = node.getStateTensor();
        // store action tensor value got from the game wrapper
        float [] actionTensor = new float[wrapper.numDistinctActions()];
        //check temp
        if(temp==1) {
...
//            StatePrinter.logState2D(node.getContext());
            ...
            for (int i = 0; i < childNodeList.size(); i++) {
                ...
                actionTensor[wrapper.moveToInt(childNodeList.get(i).getMoveFromParent())] = (float) childNodeList.get(i).getN() / sum;
                //find next
            ...
            if(!childNodeList.isEmpty()) {
                ...
                actionTensor[wrapper.moveToInt(childNodeList.get(choosedindex).getMoveFromParent())] = 1.0f;
                //set nextMove
