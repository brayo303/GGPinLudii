 ...
 */
public final class CustomStateWrapper
{
	
	//-------------------------------------------------------------------------
	
	/** Reference back to our wrapped Ludii game */
	protected CustomGameWrapper game;
	 ...
	 * @param game
	 */
	public CustomStateWrapper(final CustomGameWrapper game,Context context)
	{
		this.game = game;
	 ...
	 * @param other
	 */
	public CustomStateWrapper(final CustomStateWrapper other)
	{
	...
	/**
	 * Copies data from given other CustomStateWrapper.
	 * TODO can optimise this if we provide optimised copyFrom implementations
	 * for context and trial!
	 * 
	 * @param other
	 */
	public void copyFrom(final CustomStateWrapper other)
	{
		this.game = other.game;
		this.context = new Context(other.context);
		this.trial = this.context.trial();
	...
	@Override
	public CustomStateWrapper clone()
	{
		return new CustomStateWrapper(this);
	}
	...
	public float[] toTensorFlat()
	{
		...
		else
		{
			...
			for (int c = 0; c < containers.length; ++c)
			{
				...
				for (int site = 0; site < cont.numSites(); ++site)
				{
					...
					if (stackSize > 0)
					{
						...
						for (int i = 0; i < CustomGameWrapper.NUM_STACK_CHANNELS / 2; ++i)
						{
							if (i >= stackSize)
								break;
							
							final int what = cs.whatCell(contStartSite + site, i);
							final int channel = currentChannel + ((what - 1) * CustomGameWrapper.NUM_STACK_CHANNELS + i);
						...
						for (int i = 0; i < CustomGameWrapper.NUM_STACK_CHANNELS / 2; ++i)
						{
							...
							final int channel = 
									currentChannel + ((what - 1) * CustomGameWrapper.NUM_STACK_CHANNELS + 
											(CustomGameWrapper.NUM_STACK_CHANNELS / 2) + i);
							flatTensor[yCoords[contStartSite + site] + tensorDimY * (xCoords[contStartSite + site] + (channel * tensorDimX))] = 1.f;
						}
						
						// Finally a non-binary channel storing the height of stack
						final int channel = currentChannel + CustomGameWrapper.NUM_STACK_CHANNELS * numPieceTypes;
			...
			currentChannel += CustomGameWrapper.NUM_STACK_CHANNELS * numPieceTypes + 1;
		}
		...
		if (usesState)
		{
			...
			for (int c = 0; c < containers.length; ++c)
			{
				...
				for (int site = 0; site < cont.numSites(); ++site)
				{
					final int state = Math.min(cs.stateCell(contStartSite + site), CustomGameWrapper.NUM_LOCAL_STATE_CHANNELS - 1);
					flatTensor[yCoords[contStartSite + site] + tensorDimY * (xCoords[contStartSite + site] + ((currentChannel + state) * tensorDimX))] = 1.f;
				}
			}
			
			currentChannel += CustomGameWrapper.NUM_LOCAL_STATE_CHANNELS;
		}
