 ...
 */
public class CustomGameWrapper
{
	...
	private static Map<GameWrapperCacheKey, CustomGameWrapper> gameWrappersCache = 
			new HashMap<GameWrapperCacheKey, CustomGameWrapper>();
	 ...
	 */
	public synchronized static CustomGameWrapper construct(final String gameName)
	{
		final GameWrapperCacheKey key = new GameWrapperCacheKey(gameName, new ArrayList<String>());
		CustomGameWrapper wrapper = gameWrappersCache.get(key);
		
		if (wrapper == null)
		{
			wrapper = new CustomGameWrapper(GameLoader.loadGameFromName(gameName));
	 ...
	 * @param gameName
	 * @param gameOptions
	 * @return Returns CustomGameWrapper for give game name and options
	 */
	public static CustomGameWrapper construct(final String gameName, final String... gameOptions)
	{
		...
		if (wrapper == null)
		{
			wrapper = new CustomGameWrapper(GameLoader.loadGameFromName(gameName, Arrays.asList(gameOptions)));
			gameWrappersCache.put(key, wrapper);
		}
	...
	/**
	 * @param file
	 * @return Returns CustomGameWrapper for .lud file
	 */
	public static CustomGameWrapper construct(final File file)
	{
		final Game game = GameLoader.loadGameFromFile(file);
		return new CustomGameWrapper(game);
	}
	 ...
	 */
	public static CustomGameWrapper construct(final File file, final String... gameOptions)
	{
	 ...
	 * @param game
	 */
	public CustomGameWrapper(final Game game)
	{
		this.game = game;
	...
	public int[] moveToTensor(final Move move)
	{
		...
		else
		{
			...
			if (game.isStacking())
			{
				channelIdx *= (CustomGameWrapper.MOVE_TENSOR_LEVEL_CLIP + 1);
				channelIdx += MathRoutines.clip(levelMin, 0, CustomGameWrapper.MOVE_TENSOR_LEVEL_CLIP);

				channelIdx *= (CustomGameWrapper.MOVE_TENSOR_LEVEL_CLIP + 1);
				channelIdx += MathRoutines.clip(levelMax - levelMin, 0, CustomGameWrapper.MOVE_TENSOR_LEVEL_CLIP);
			}
	...
	{
		final int[] moveTensorsShape = moveTensorsShape();
		return moveTensorsShape[1] * moveTensorsShape[2] * moveTensorsShape[0];
	}
	 ...
	 */
	public int[] moveTensorSourceChannels(final CustomGameWrapper sourceGame)
	{
			...
			else
			{
				...
				if ((game.gameFlags() & GameType.UsesFromPositions) == 0L)
				{
					...
					else
					{
						...
						// channels unused.
						if (targetChannel != 0)
							throw new UnsupportedOperationException("CustomGameWrapper::moveTensorSourceChannels() expected targetChannel == 0!");
	 ...
	 * @return Indices of source channels we should transfer from
	 */
	public int[] stateTensorSourceChannels(final CustomGameWrapper sourceGame)
	{
	...
	public static void main(final String[] args)
	{
		...
		for ( ; i < gameNames.length ; i++)
		{
			...
			StatePrinter.logln(i+". name = " + name);
			final CustomGameWrapper game = CustomGameWrapper.construct(name);
			
			System.out.println(name);
			if (!game.game.hasSubgames())
			{
				StatePrinter.logln("State tensor shape = " + Arrays.toString(game.stateTensorsShape()));
				StatePrinter.logln("Moves tensor shape = " + Arrays.toString(game.moveTensorsShape()));
			}
			}
			catch(Exception e) {
